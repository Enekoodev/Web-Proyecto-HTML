...
cars.color = function (c) {
  ...
  let randomLight = 2.5 + (Math.random() * 2);
...
road.frame = function () {
  ...
  road.ctx.bezierCurveTo(road.Pc.x1, road.P1.y - (road.height*0.7), road.P0.x, road.P0.y,road.P0.x, road.P0.y);
  
  road.ctx.moveTo(road.P2.x,  road.P2.y);
  road.ctx.bezierCurveTo(road.Pc.x2, road.P2.y - (road.height*0.7), road.P0.x, road.P0.y, road.P0.x, road.P0.y);
...
mountains.frame = function () {
  let curve = (road.P0.x - road.width/2)/100; 
...
let clickstart = document.getElementById('click')
